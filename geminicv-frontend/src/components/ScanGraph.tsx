...
const ScanGraph: React.FC<ScanCardProps> = ({ scans }) => {
  ...
  const annualGrossSalaryMinData = scans?.map(
    (scan) => scan.anual_gross_salary_min
  );
  const annualGrossSalaryAvgData = scans?.map(
    (scan) => scan.anual_gross_salary_avg
  );

  const hourlyFreelanceRateMinData = scans?.map(
    (scan) => scan.hourly_freelance_rate_min
  );
  const hourlyFreelanceRateAvgData = scans?.map(
    (scan) => scan.hourly_freelance_rate_avg
  );
